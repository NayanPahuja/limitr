...
impl RedisClientTrait for RedisClientImpl {
    async fn execute_rate_limit_script(
        ...
        let mut conn = self.pool.get().await.map_err(|e| {
            ...
            crate::metrics::record_redis_error("connection_establishment_failed");
        ...
        let result: Vec<redis::Value> = script
            ...
            crate::metrics::record_redis_error("script_execution_failed");
            crate::metrics::record_script_execution(false);
            RateLimitError::ScriptExecutionError(format!("Script execution failed: {}", e))
        })?;

        // Record metrics
        let duration = start.elapsed().as_secs_f64();
        crate::metrics::record_redis_duration("script_execution_latency", duration);
        crate::metrics::record_script_execution(true);
            ...
            redis::Value::BulkString(bytes) => {
                let s = std::str::from_utf8(bytes).map_err(|e| {
                    RateLimitError::ScriptExecutionError(format!(
                        "Invalid UTF-8 in capacity: {}",
                        e
                    ))
                })?;
                s.parse::<f64>().map_err(|e| {
                    RateLimitError::ScriptExecutionError(format!("Failed to parse capacity: {}", e))
                })?
            }
            _ => {
                return Err(RateLimitError::ScriptExecutionError(format!(
                    "Invalid capacity value type: {:?}",
                    result[1]
                )));
            }
        };
    ...
    async fn health_check(&self) -> Result<()> {
        ...
        let mut conn = self.pool.get().await.map_err(|e| {
            crate::metrics::record_redis_error("health_check_failed");
            RateLimitError::RedisConnectionError(redis::RedisError::from((
                redis::ErrorKind::IoError,
                "Pool exhausted",
                e.to_string(),
        ...
        let response: String = redis::cmd("PING")
            ...
            .map_err(|e| {
                crate::metrics::record_redis_error("health_check_failed");
                RateLimitError::RedisConnectionError(e)
            })?;

        let duration = start.elapsed().as_secs_f64();
        crate::metrics::record_redis_duration("ping", duration);
